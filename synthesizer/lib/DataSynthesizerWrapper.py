...
def generate_data(username):
    ...
    if configuration['chose_mode'] == 'mode1':
        ...
        describer.describe_dataset_in_random_mode(input_dataset_file, {}, attribute_to_is_categorical,
                                                  attribute_to_is_candidate, seed=seed)
        ...
        if configuration['chose_mode'] == 'mode2':
            describer.describe_dataset_in_independent_attribute_mode(input_dataset_file, epsilon, attribute_to_datatype,
                                                                     attribute_to_is_categorical, attribute_to_is_candidate,
                                                                     seed=seed)
            describer.save_dataset_description_to_file(description_file)
            ...
            with open(f'{username}_code.py', 'w') as f:
                code_str = f'''
from DataSynthesizer.DataDescriber import DataDescriber
...
describer.save_dataset_description_to_file('{description_file}')
            ...
            subprocess.run(f'python {username}_code.py', shell=True)
